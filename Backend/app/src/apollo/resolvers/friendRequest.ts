export const resolvers = {
	...
	Query: {
		incomingFriendRequests: async (parent, args, { prisma }) =>
			await prisma.user({ id: args.data.userID }).incomingFriendRequests(),
		outgoingFriendRequests: async (parent, args, { prisma }) =>
			await prisma.user({ id: args.data.userID }).outgoingFriendRequests(),
	},
	FriendRequest: {
		fromUser: async (parent, args, { prisma }) =>
			await prisma.friendRequest({ id: parent.id }).fromUser(),
		toUser: async (parent, args, { prisma }) =>
			await prisma.friendRequest({ id: parent.id }).toUser(),
	},
};
